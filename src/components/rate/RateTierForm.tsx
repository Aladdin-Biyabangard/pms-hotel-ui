...
import {CreateRateTierRequest, UpdateRateTierRequest, rateTierApi, RateTierResponse} from '@/api/rateTier';
...
export function RateTierForm({open, onClose, onSuccess, initialData, defaultRatePlanId}: RateTierFormProps) {
    ...
    useEffect(() => {
        const loadRatePlans = async () => {
            try {
                const data = await ratePlanApi.getAllRatePlans(0, 10);
    ...
    const onSubmit = async (data: RateTierFormValues) => {
        ...
        try {
            ...
            if (isEditMode && initialData) {
                await rateTierApi.updateRateTier(initialData.id, payload as UpdateRateTierRequest);
                toast.success('Rate tier updated successfully');
            } else {
                await rateTierApi.createRateTier(payload as CreateRateTierRequest);
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>{isEditMode ? 'Edit Rate Tier' : 'Create Rate Tier'}</DialogTitle>
                </DialogHeader>
                <Form {...form}>
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        ...
                                        >
                                            ...
                                            <SelectContent>
                                                {ratePlans.map(rp => (
                                                    <SelectItem key={rp.id}
                                                                value={rp.id.toString()}>{rp.name}</SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                        <FormMessage/>
                            ...
                            <FormField
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        ...
                                        <FormDescription>
                                            {form.watch('adjustmentType') === 'PERCENTAGE' && 'Percentage value (e.g., 10 for 10%)'}
                                            {form.watch('adjustmentType') === 'FIXED' && 'Fixed amount (e.g., 50.00)'}
                                            {form.watch('adjustmentType') === 'MULTIPLIER' && 'Multiplier value (e.g., 1.2 for 20% increase)'}
                                        </FormDescription>
                                        <FormMessage/>
                                    </FormItem>
                                )}
...
// Pagination component for Rate Plans
function RatePlanPagination() {
    const [ratePlans, setRatePlans] = useState<RatePlanResponse[]>([]);
    ...
    useEffect(() => {
        const loadRatePlans = async () => {
            setIsLoading(true);
            try {
                const data = await ratePlanApi.getAllRatePlans(currentPage, pageSize);
                setRatePlans(data.content);
        ...
        <div className="mt-4">
            <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-2">
                    <Button size="sm" variant="outline" onClick={() => setCurrentPage(Math.max(0, currentPage - 1))}
                            disabled={currentPage === 0 || isLoading}>Previous</Button>
                    <Button size="sm" variant="outline" onClick={() => setCurrentPage(currentPage + 1)}
                            disabled={currentPage >= totalPages - 1 || isLoading}>Next</Button>
                </div>
            </div>
            <ul>
                {ratePlans.map(rp => (
                    <li key={rp.id} className="border-b p-2">{rp.name}</li>
                ))}
            </ul>
        </div>
    );
