...
export function RateClassList({ onEdit, onDelete, onCreate }: RateClassListProps) {
  ...
  const [filteredRateClasses, setFilteredRateClasses] = useState<RateClassResponse[]>([]);
  ...
  useEffect(() => {
    filterRateClasses();
  }, [searchTerm, filterStatus, rateClasses]);
  ...
  const loadData = async () => {
    try {
      ...
      if (filterCategory !== 'all') {
        params.rateCategoryId = parseInt(filterCategory, 10);
      }
      const data = await rateClassApi.getAllRateClasses(0, 1000, params);
      setRateClasses(data.content);
    } catch (error) {
      console.error('Failed to load rate classes', error);
      toast.error('Failed to load rate classes');
    } finally {
      setIsLoading(false);
    }
  };

  const filterRateClasses = () => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        rc =>
          rc.code?.toLowerCase().includes(term) ||
          rc.name?.toLowerCase().includes(term) ||
          rc.description?.toLowerCase().includes(term) ||
          rc.rateCategoryName?.toLowerCase().includes(term)
      );
    }
    ...
    if (filterStatus !== 'all') {
      filtered = filtered.filter(rc => {
        if (filterStatus === 'active') return rc.status === 'ACTIVE';
        if (filterStatus === 'inactive') return rc.status === 'INACTIVE';
        return true;
      });
    }

    setFilteredRateClasses(filtered);
  };
  ...
  const handleDelete = async (rateClass: RateClassResponse) => {
    ...
    try {
      await rateClassApi.deleteRateClass(rateClass.id);
                ...
                {filteredRateClasses.map((rateClass) => (
                  <TableRow key={rateClass.id}>
                    ...
                    <TableCell className="hidden md:table-cell">
                      {rateClass.rateCategoryName ? (
                        <Badge variant="outline">{rateClass.rateCategoryName}</Badge>
                      ) : (
